...
export default function QuickStartPage() {
  ...
  const handlePaymentAndSubmit = async (e) => {
    ...
    try {
      ...
      const orderRes = await fetch("/finvolve/api/create-order", {
        method: "POST",
      ...
      const options = {
        ...
        handler: async (response) => {
          try {
            const verifyRes = await fetch("/finvolve/api/verify-payment", {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({
                source: "quick_start",
  ...
  return (
    <div className="min-h-screen px-6 py-12">
      <div className="container">
        <Link href="/finvolve" className="mb-8 inline-flex items-center text-sm font-medium text-slate-500 transition-colors hover:text-primary">
        ...
        <div className="mx-auto max-w-5xl">
          ...
          {status === "success" ? (
            <Card hover={false} className="glass-surface-strong mx-auto max-w-lg text-center">
              ...
              <Button href="/finvolve" variant="primary">Return Home</Button>
            </Card>
          ) : (
