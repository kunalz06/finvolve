...
function PaymentPortalContent() {
  ...
  useEffect(() => {
    ...
    const loadSession = async () => {
      if (!token) {
        setError("Missing payment token. Please use the payment link sent by Finvolve.");
        setLoading(false);
        return;
      }
      ...
      setError("");
      try {
        const response = await fetch("/finvolve/api/payment-session", {
          method: "POST",
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      const orderResponse = await fetch("/finvolve/api/create-order", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      const options = {
        ...
        handler: async (response) => {
          try {
            const verifyResponse = await fetch("/finvolve/api/verify-payment", {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({
