      ...
      <div className="min-h-screen px-4 py-8 sm:px-6 sm:py-12">
        <div className="container">
          <div className="glass-surface-strong mx-auto max-w-lg rounded-2xl px-6 py-12 sm:px-8 sm:py-16 text-center">
            <div className="mx-auto mb-6 sm:mb-8 flex h-20 w-20 sm:h-24 sm:w-24 items-center justify-center rounded-full bg-emerald-100/90">
          ...
          <div className="grid gap-6 sm:gap-8 lg:grid-cols-5">
            <div className="space-y-4 sm:space-y-6 lg:col-span-2">
                ...
                <div className="space-y-4 sm:space-y-6">
                  ...
                  <div className="flex items-start gap-3 sm:gap-4">
                    <div className="glass-icon-plate flex h-10 w-10 sm:h-12 sm:w-12 flex-shrink-0 items-center justify-center rounded-xl">
                      <MapPin className="text-slate-500 size-5 sm:size-[20px]" />
                    </div>
                    <div>
              ...
              <Card hover={false} className="glass-surface-strong">
                ...
                <h3 className="mb-2 text-base sm:text-lg font-bold text-slate-950">Ready to Start?</h3>
                ...
                <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
                  <div className="grid gap-4 sm:gap-6 md:grid-cols-2">
                    <div>
                      ...
                      <input type="text" id="name" name="name" value={formData.name} onChange={handleChange} required placeholder="John Doe" className="w-full rounded-xl px-3 py-2.5 sm:px-4 sm:py-3 text-slate-900" />
                    </div>
                    <div>
                      <label htmlFor="email" className="mb-2 block text-sm font-medium text-slate-700">Email Address</label>
                      <input type="email" id="email" name="email" value={formData.email} onChange={handleChange} required placeholder="john@example.com" className="w-full rounded-xl px-3 py-2.5 sm:px-4 sm:py-3 text-slate-900" />
                    </div>
                  </div>

                  <div>
                    <label htmlFor="subject" className="mb-2 block text-sm font-medium text-slate-700">Subject</label>
                    <input type="text" id="subject" name="subject" value={formData.subject} onChange={handleChange} required placeholder="How can we help?" className="w-full rounded-xl px-3 py-2.5 sm:px-4 sm:py-3 text-slate-900" />
                  </div>

                  <div>
                    <label htmlFor="message" className="mb-2 block text-sm font-medium text-slate-700">Message</label>
                    <textarea id="message" name="message" rows={4} value={formData.message} onChange={handleChange} required placeholder="Tell us about your project..." className="w-full resize-none rounded-xl px-3 py-2.5 sm:px-4 sm:py-3 text-slate-900" />
                  </div>

                  {status === "error" && (
                    <div className="flex items-center gap-3 rounded-[22px] border border-red-200 bg-red-50/85 p-3 sm:p-4 text-red-700">
