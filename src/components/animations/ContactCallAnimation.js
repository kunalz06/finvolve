...
export default function ContactCallAnimation({ className = "", onClose }) {
  ...
  const receiverCordRef = useRef(null);
  ...
  const callBubble1Ref = useRef(null);
  const callBubble2Ref = useRef(null);
  const callBubble3Ref = useRef(null);

  useEffect(() => {
    const ringTimer = setTimeout(() => setIsRinging(true), 500);
    const talkTimer = setTimeout(() => {
      setIsRinging(false);
      setIsTalking(true);
    }, 2500);
    return () => { clearTimeout(ringTimer); clearTimeout(talkTimer); };
  }, []);
    ...
    <div className={`fixed inset-0 z-50 flex items-center justify-center bg-black/20 backdrop-blur-sm ${className}`}>
      ...
      <div className="relative w-full max-w-3xl px-4">
        ...
        <div className="relative flex items-end justify-center h-96">
          <div ref={telephoneRef} className="relative">
            ...
            <div ref={receiverRef} className="absolute -top-8 left-1/2 -translate-x-1/2" style={{ transformOrigin: "left center" }}>
              <div className="w-32 h-10 bg-[var(--surface-strong)] border-2 border-[var(--border)] rounded-full shadow-xl">
                <div className="absolute inset-1 grid grid-cols-8 gap-0.5">
                  {[...Array(8)].map((_, i) => <div key={i} className="bg-[var(--border)] rounded-sm" />)}
                </div>
              ...
              <div ref={receiverCordRef} className="absolute -left-32 top-2 w-32 h-1 bg-[var(--border)] rounded-full origin-right" style={{ transformOrigin: "right center", opacity: 0 }} />
            </div>
            <div className="absolute bottom-6 left-6 right-6 grid grid-cols-3 gap-3">
              {[[1,2,3],[4,5,6],[7,8,9],["*",0,"#"]].flat().map((num,i)=><div key={i} className="w-10 h-10 bg-[var(--surface-muted)] border border-[var(--border-soft)] rounded-full flex items-center justify-center text-sm font-bold text-[var(--foreground)]">{num}</div>)}
            </div>
            <div className="absolute top-12 left-6 right-6 h-8 bg-[var(--red-primary)] border-2 border-[var(--border)] rounded-xl flex items-center justify-center px-4">
              <span ref={displayTextRef} className="text-sm font-bold text-[var(--surface)]">{isTalking?"CALLING...":isRinging?"RINGING":"READY"}</span>
            </div>
            ...
            <div className="absolute bottom-4 left-2 text-[8px] font-bold text-[var(--muted)] rotate-90 origin-center">DEV</div>
            <div className="absolute -bottom-16 left-1/2 -translate-x-1/2 w-1.5 h-24 bg-[var(--border)] rounded-full" />
          </div>
          <div ref={personRef} className="absolute -right-40 top-8" style={{ transform: "translateX(50px)", opacity: 0 }}>
            <div className="relative">
              <div ref={headRef} className="relative w-14 h-14 rounded-full bg-[var(--red-primary)] border-2 border-[var(--border)] shadow-lg">
                ...
                <div ref={mouthRef} className="absolute bottom-4 left-1/2 -translate-x-1/2 w-6 h-3 border-b-2 border-[var(--surface)] rounded-full" style={{ origin: "center bottom", width: 24 }} />
              ...
              <div ref={bodyRef} className="w-16 h-24 bg-[var(--red-light)] border-2 border-[var(--border)] rounded-b-2xl mx-auto relative shadow-md" style={{ opacity: 0 }}>
                <div className="absolute top-4 left-2 w-5 h-5 bg-[var(--red-primary)] border border-[var(--border)] rounded-sm" />
                <div ref={leftArmRef} className="absolute -right-14 top-8 w-16 h-3 bg-[var(--red-light)] border-2 border-[var(--border)] rounded-full origin-right" style={{ transformOrigin: "right center" }}>
              ...
              <div ref={legsRef} className="flex justify-center gap-1 mt-1" style={{ opacity: 0 }}>
                <div className="w-5 h-14 bg-[var(--surface-strong)] border-2 border-[var(--border)] rounded-full" />
                <div className="w-5 h-14 bg-[var(--surface-strong)] border-2 border-[var(--border)] rounded-full" />
              </div>
          ...
          <div ref={callBubble1Ref} className="absolute -top-12 left-8" style={{ scale: 0, opacity: 0 }}>
            <div className="relative"><div className="w-20 h-14 bg-[var(--red-soft)] border-2 border-[var(--border)] rounded-2xl" /><div className="absolute -bottom-3 left-3 w-5 h-5 bg-[var(--red-soft)] border-2 border-b-transparent border-l-2 border-r-2 border-[var(--border)] rounded-bl-2xl" /></div>
          </div>
          <div ref={callBubble2Ref} className="absolute -top-20 right-12" style={{ scale: 0, opacity: 0 }}>
            <div className="relative"><div className="w-16 h-10 bg-[var(--red-soft)] border-2 border-[var(--border)] rounded-2xl opacity-80" /><div className="absolute -bottom-3 right-3 w-4 h-4 bg-[var(--red-soft)] border-2 border-b-transparent border-l-2 border-r-2 border-[var(--border)] rounded-br-2xl opacity-80" /></div>
          </div>
          <div ref={callBubble3Ref} className="absolute -top-16 left-1/2 -translate-x-1/2" style={{ scale: 0, opacity: 0 }}>
            <div className="relative"><div className="w-12 h-8 bg-[var(--red-soft)] border-2 border-[var(--border)] rounded-2xl opacity-60" /><div className="absolute -bottom-2 left-1/2 -translate-x-1/2 w-3 h-3 bg-[var(--red-soft)] border-2 border-b-transparent border-l-2 border-r-2 border-[var(--border)] rounded-b-2xl opacity-60" /></div>
          </div>
        </div>
      </div>
    </div>
